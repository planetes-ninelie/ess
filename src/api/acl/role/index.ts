...
enum API {
  //获取所有角色数据url | 表格数据url
  GETROLESDATA_URL = '/roles/list',
  //新增角色url
  ADDROLE_URL = '/roles/add',
  //修改角色url
  UPDATEROLE_URL = '/roles/edit',
  //根据id删除角色url
  DELETEROLE_URL = '/roles/delete/',
  //根据id列表删除角色url
  DELETEROLELIST_URL = '',
}
...
export const getRolesData = (page: number, limit: number, roleName: string) => {
  ...
  return request.post<any, GetRoleResponseData>(API.GETROLESDATA_URL, dto)
}
